  ...
  async run(message, args) {
    ...
    if (talkedRecently.has(message.author.id)) {
      message.channel.sendCustom(
        `You are **blocked** from the following command - ${message.author}`
      );
    } else {
      ...
      if (
        link.includes("porn") ||
        ipLoggers.includes(link) ||
        link.includes("sex") ||
        link.includes("grabify") ||
        link.includes("iplogger") ||
        link.includes("2no") ||
        link.includes("yip") ||
        link.includes("iplis") ||
        link.includes("02ip") ||
        link.includes("ezstat") ||
        link.includes("logger")
      )
        return (
          message.channel.sendCustom(
            `${message.author} ${language.shorturlBlock1}`
      ...
      message.channel.sendCustom({
        embeds: [
          new discord.MessageEmbed()
            .setDescription(
              `${language.urlCreated}\n\n**Short Url:** [https://kysta.tech/url/${kaka}](https://kysta.tech/url/${kaka})\n**Full url:** ${args[0]}\n\n**Please note that by making urls you abide by our [policy](https://kysta.tech/url)**`
            )
