...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    const green = '<:arrow_arrow:1086900523757740082> ';
    const red = '<:Failed:1086876721703288872>';
    ...
    if (!args || args.length < 1) {
      ...
      embed.setTitle(`Kysta's Command List`);
      embed.setDescription(stripIndent`
      <:arrow_arrow:1086900523757740082> The Prefix for this server is \`${prefix}\`
      ...
      embed.setThumbnail(`https://Kysta.tech/logo.png`);
      embed.setFooter(
        cmd.disabled ||
      ...
      if (cmd.examples && cmd.examples.length)
        embed.addField(
          "__**Examples**__",
          cmd.examples
            .map((example) => `<:arrow_arrow:1086900523757740082> \`${example}\``)
            .join("\n")
        );
