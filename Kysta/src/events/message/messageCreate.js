    ...
    try {
      ...
      if (message.content.match(mentionRegex)) {
        ...
        const embed = new MessageEmbed()
          .setTitle("Hello, I'm Kysta. What's Up?")
          .addField(`Prefix`, proofita, true)
          .addField(`Usage`, proofitaa, true)
          .setDescription(
            `\nIf you like Kysta, Consider [voting](https://top.gg/bot/), or [inviting](${config.invite_link}) it to your server! Thank you for using Kysta, we hope you enjoy it, as we always look forward to improve the bot`
          )
          .setFooter("Thank you for using Kysta!!")
          .setColor("#FF2C98");
      ...
      if (!this.client.config.developers.includes(message.author.id)) {
        if (maintenance && maintenance.toggle == "true") {
          ...
          message.channel.sendCustom(
            `Kysta is currently undergoing maintenance which won't allow anyone to access Kysta's Commands. Feel free to try again later. For updates: ${config.discord}`
          );
          ...
          return message.channel
            .sendCustom(
              ` ${
                ...
                number === 1
                  ? "*Did You know that Kysta has its own dashboard? `https://Kysta.tech/dashboard`*"
                  : ""
              }${
                number === 2
                  ? "*You can check our top.gg page at `https://vote.Kysta.tech`*"
                  : ""
              }`
            )
            .then((s) => {
