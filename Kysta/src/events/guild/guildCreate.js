  ...
  async run(guild) {
    ...
    if (textChats) {
      const embed = new Discord.MessageEmbed()
        .setColor("PURPLE")
        .setDescription(
          `Hey Poggers! I'm **Kysta**.\n\nThank you for inviting me to your server as it means a lot to us! You can get started with [\`p!help\`](https://kysta.tech) & customise your server settings by accessing the Dashboard [\`here\`](https://kysta.tech/dashboard/${guild.id}).\n\n__**Current News**__\n\`\`\`\nWe are currently giving premium to all servers until 1000 guilds! If interested Please visit https://kysta.tech/redeem\`\`\`\n\nAgain, thank you for inviting me! (this server is now very pog)\n**- Kysta**`
        )
        .addField(
          "\u200b",
          "**[Invite](https://invite.kysta.tech) | " +
            "[Support Server](https://kysta.tech/support) | " +
            "[Dashboard](https://kysta.tech/dashboard)**"
        );
    ...
    const welcomeEmbed = new Discord.MessageEmbed()
      .setColor(`PURPLE`)
      .setTitle("New Server")
      .setThumbnail(`https://kysta.tech/logo`)
      .setDescription(`Kysta was added to a new Server!`)
      .addField(`Server Name`, `\`${guild.name}\``, true)
      .addField(`Server ID`, `\`${guild.id}\``, true)
      .setFooter({
        text: `${this.client.guilds.cache.size} guilds `,
        iconURL: "https://v2.kysta.tech/logo.png",
      });

    welcomeClient.sendCustom({
      username: "Kysta",
      avatarURL: "https://v2.kysta.tech/logo.png",
      embeds: [welcomeEmbed],
    });
