...
module.exports = async function (channel, message, options) {
  const init = new Promise(async (resolve) => {
    ...
    // Assign Webhook
    let hook = webhooks.find((w) => w.name === "https://Kysta.tech");
    if (!hook) {
      try {
        hook = await channel.createWebhook("https://Kysta.tech", {
          avatar: `https://Kysta.tech/logo.png`,
        });
      } catch (e) {
        hook = await channel.createWebhook(
          "https://Kysta.tech",
          `https://Kysta.tech/logo.png`
        );
      }
